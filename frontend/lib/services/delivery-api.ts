...
export const deliveryApi = api.injectEndpoints({
	...
	endpoints: (builder) => ({
		...
		syncDriverProfiles: builder.mutation<
		...
		getDriverEarnings: builder.query<DriverEarnings, number>({
			query: (userId) => ({
				url: `${DELIVERY_BASE_URL}/driver/${userId}/earnings`,
				method: "GET",
			}),
			providesTags: (_result, _error, userId) => [
				{ type: "Delivery", id: `EARNINGS_${userId}` },
			],
		}),
